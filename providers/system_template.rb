...
def download_systemvm_template
  ...
  download_cmd = "{#node['cloudstack']['cloud-install-sys-tmplt']}  -m #{@current_resource.nfs_path} -u #{@current_resource.url} -h #{@current_resource.hypervisor} -t #{@current_resource.hypervisor} -F"
